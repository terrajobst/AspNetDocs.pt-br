...
## <a name="additional"></a>Adicional
  * Aperfeiçoamento dos algoritmos cifES e nieES. Fecha #1826

## <a name="build"></a>Build
  * Inclusão da versão reduzida no Pacote NPM
  * Elevação das dependências de desenvolvimento para versões mais recentes

## <a name="core"></a>Núcleo
  * Adição de associação para entrada com tipo de botão. Fecha #1891
  * Suporte ao jquery3. Fecha #1866
  * Alteração de jQuery alias 'expr[":"]' para 'expr.pseudos'

## <a name="localisation"></a>Localização
  * Adição de tradução em urdu. Fecha #1873.

## <a name="localization"></a>Localização
  * Correção de extensão de arquivo incorreta para tradução az. Fecha #1890.
  * Adição de tradução ausente em pt-BR (fecha #1897)
  * Correção de erro de digitação no arquivo de idioma árabe.

## <a name="tests"></a>Testes
  * Atualização do QUnit para 2.0.

## <a name="umd"></a>UMD
  * Melhor suporte para CommonJS.

<a name="1151--2016-07-22"></a>1.15.1 / 2016-07-22
==================

## <a name="additional"></a>Adicional
  * Correção de validação múltipla do tipo mime
  * O IBAN requer pelo menos 5 caracteres (fecha #1797, corrige #1674)
  * Referência de jQuery correta notEqualTo

## <a name="core"></a>Núcleo
  * Adição de teste com falha para #1805
  * Correção de validação de grupo com 3 e mais campos
  * Correção de regressões introduzidas no #1644 e #1657 (fecha # 1800)
  * Atualização da validação da etapa para lidar corretamente com pontos flutuantes
  * Correção de erro ao chamar $.fn.rules() em um elemento não formulário
  * Chamada de `errorPlacement` no escopo do validador
  * Correção do problema com elementos ​​de conteúdo editável em formulários em que os eventos para validação de entrada única causariam exceções

## <a name="demo"></a>Demonstração
  * Adição de links para demos do Bootstrap e do Semantic-UI para index.html
  * Use `.on()` em vez de `.validateDelegate()`

## <a name="localization"></a>Localização
  * Adição de idioma azeri

## <a name="tests"></a>Testes
  * Adição de testes de unidade de regressão para PR #1760

<a name="1150--2016-02-24"></a>1.15.0 / 2016-02-24
==================

## <a name="all"></a>Todos
  * Correção de problemas de estilo de código

## <a name="core"></a>Núcleo
  * `resetForm` também deve remover a classe `valid` dos elementos.
  * Cancelamento do realce do campo se este já estiver destacado ao usar regra remota.
  * Associação ao evento de `blur` apenas uma vez na regra `equalTo`
  * Correção de erro ao chamar .rules() no conjunto jquery vazio.
  * Correção na manipulação de mensagens de erro com grupos de entrada.
  * Correção de TypeError em `showLabel` ao usar `groups`as configurações
  * Adição de uma maneira de passar o nome do método para remoto
  * Falha na validação quando o campo seguinte já está preenchido (corrige #1508)
  * A regra obrigatória tem precedência sobre as regras de números e dígitos
  * Erro oculto, mas a classe de erro de entrada não foi removida
  * Validação remota usa mensagens de erro erradas
  * Correção de realce de campo com validação remota.
  * Correção do seletor `:filled` para vários elementos selecionados.
  * Adição de referência de doc para jQuery.validator.methods
  * Mudança do processamento de mensagens de `formatAndAdd` para `defaultMessage`
  * ErrorList deve conter apenas os erros que deveria
  * Extração do nome do arquivo sem incluir "C:\fakepath\"
  * Suporte ao atributo de etapa HTML5. Corrige #1295
  * Adição de suporte para classe "pendente" em solicitações pendentes
  * Adição de normalizador (#1602)
  * Divisão do método `creditcard`
  * Escape errorID para uso no regex, não para construir aria-describedby
  * Escape de cotações simples em nomes evitando um erro Sizzle sendo lançado
  * Em vez de usar a validação do valor do campo para ignorar a chamada da API, use o objeto de dados serializados da solicitação
  * Adição de suporte para tags contentEditable

## <a name="additional"></a>Adicional
  * BIC: permite dígitos de 1-9 em segundo lugar de localização
  * Aceitação de que o método regex deve ser ignorado corretamente.
  * Verificação insensível a maiúsculas e minúsculas para BIC
  * PostalCodeCA correto para excluir combinações inválidas
  * Tornar o método postalCodeCA mais leniente

## <a name="localization"></a>Localização
  * Adição da localização Macedônio.
  * Adição de mensagem padrão ausente em polonês (adamwojtkiewicz)
  * Correção da tradução persa da mensagem mín/máx.
  * Atualização de messages_sk.js
  * Atualização da tradução em malaio
  * Mensagens incluídas de métodos adicionais
  * Aprimoramento da tradução pt_BR e correção de um erro de digitação na chave 'cifES'.
...
## <a name="core"></a>Núcleo
  ...
  * Não pesquisar no DOM inteiro por elementos de opção/caixa de seleção
...
* Não use form.elements, pois ele não tem mais suporte no jQuery 1.6. Ele é cheio de bugs mesmo (IE6-8: form.elements === form).
...
* Correção de métodos opcionais para não realçar coisa alguma se o campo estiver em branco, ou seja, não disparar o êxito
...
* Remoção completa da dependência em IDs de elemento, embora ainda sejam usadas (quando houver) para vincular rótulos de erro às entradas. Obtenção por meio de uma matriz com {name, message, element} em vez de um objeto com pares de id:message para errorList interno.
...
* API simplificada para oferecer suporte a diferentes eventos. O padrão, enviar, pode ser desabilitado. Se nenhum evento for especificado, isso será aplicado a cada elemento (em vez de todo o formulário). A combinação de keyup-validation com submit-validation agora é extremamente fácil de configurar
