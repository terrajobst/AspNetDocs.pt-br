...
Para encaminhar os cabeçalhos `X-Forwarded-For` e `X-Forwarded-Proto`, consulte <xref:host-and-deploy/linux-nginx#configure-nginx>. Para obter mais informações, consulte [NGINX: Usando o cabeçalho Forwarded](https://www.nginx.com/resources/wiki/start/topics/examples/forwarded/).

## <a name="apache-configuration"></a>Configuração do Apache

`X-Forwarded-For` é adicionado automaticamente (consulte [mod_proxy do módulo Apache: Inverter os cabeçalhos de solicitação de Proxy](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html#x-headers)). Para obter informações sobre como encaminhar o cabeçalho `X-Forwarded-Proto`, consulte <xref:host-and-deploy/linux-apache#configure-apache>.
