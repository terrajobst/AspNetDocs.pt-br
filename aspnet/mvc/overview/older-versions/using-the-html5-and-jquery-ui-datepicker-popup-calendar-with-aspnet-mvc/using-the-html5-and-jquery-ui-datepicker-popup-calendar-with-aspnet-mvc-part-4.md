...
Observação: Se o **ferramentas** menu não exibe o **Gerenciador de pacotes NuGet** de comando, você precisa instalar o NuGet, seguindo as instruções no [instalando o NuGet](http://docs.nuget.org/docs/start-here/installing-nuget) página do Site do NuGet.   
...
Observação: As versões de depuração (os arquivos sem as *. Min* extensão) são úteis para depuração, mas em um site de produção, você incluiria apenas as versões minimizadas.
