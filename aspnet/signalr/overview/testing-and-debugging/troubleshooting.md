...
- [Configurando WebSockets do IIS para executar ping/pingue para detectar um cliente morto](#pong)
...
### <a name="ondisconnect-not-firing-at-consistent-times"></a>OnDisconnect não está acionando em tempo consistente

Este comportamento ocorre por design. Quando um usuário tenta navegar para fora de uma página com uma conexão de Signalr ativa, o cliente do Signalr então fará uma tentativa de melhor esforço para notificar o servidor de que a conexão do cliente será interrompida. Se a tentativa de melhor esforço do cliente de sinalização falhar ao alcançar o servidor, o servidor descartará a conexão após uma `DisconnectTimeout` configurável mais tarde, quando o evento de `OnDisconnected` será acionado. Se a tentativa de melhor esforço do cliente de sinalização for bem-sucedida, o evento `OnDisconnected` será acionado imediatamente.

Para obter informações sobre como definir a configuração de `DisconnectTimeout`, consulte [manipulando eventos de tempo de vida da conexão: DisconnectTimeout](../guide-to-the-api/handling-connection-lifetime-events.md#disconnecttimeout).
...
## <a name="configuring-iis-websockets-to-pingpong-to-detect-a-dead-client"></a>Configurando WebSockets do IIS para executar ping/pingue para detectar um cliente morto

Os servidores de sinalização não sabem se o cliente está inativo ou não e dependem da notificação do WebSocket subjacente para falhas de conexão, ou seja, o retorno de chamada `OnClose`. Uma solução para esse problema é configurar o WebSockets do IIS para fazer o ping/pingue para você. Isso garante que a conexão será fechada se for interrompida inesperadamente. Para obter mais informações, consulte [esta postagem de StackOverflow](http://stackoverflow.com/questions/19502755/websocket-clients-state-not-changing-on-network-loss).
...
- **Adicionando rotas ao aplicativo antes de adicionar a rota do Hub:** Se seu aplicativo usar outras rotas, verifique se a primeira rota adicionada é a chamada para `MapSignalR`.
- **Usando o IIS 7 ou 7,5 sem a atualização para URLs com extensão:** O uso do IIS 7 ou 7,5 requer uma atualização para URLs sem extensão para que o servidor possa fornecer acesso às definições de Hub em `/signalr/hubs`. A atualização pode ser encontrada [aqui](https://support.microsoft.com/kb/980368).
...
Esse erro também é visto se um firewall ou proxy não estiver configurado corretamente, fazendo com que os cabeçalhos de solicitação sejam regravados. A solução é certificar-se de que a porta 80 esteja habilitada no firewall ou proxy.

### <a name="unexpected-response-code-500"></a>"Código de resposta inesperado: 500"

Esse erro pode ocorrer se a versão do .NET Framework usada no aplicativo não corresponder à versão especificada em Web. config. A solução é verificar se o .NET 4,5 é usado nas configurações do aplicativo e no arquivo Web. config.

### <a name="typeerror-lthubtypegt-is-undefined-error"></a>Erro "TypeError: &lt;hubType&gt; indefinido"

Esse erro será resultado se a chamada para `MapSignalR` não for feita corretamente. Consulte [como registrar o middleware do signalr e configurar as opções do signalr](../guide-to-the-api/hubs-api-guide-server.md#route) para obter mais informações.
...
### <a name="invalidoperationexception-not-a-valid-web-socket-request"></a>"InvalidOperationException: não é uma solicitação de soquete da Web válida.

Esse erro pode ocorrer se o protocolo WebSocket for usado, mas o proxy de rede estiver modificando os cabeçalhos de solicitação. A solução é configurar o proxy para permitir WebSocket na porta 80.

### <a name="exception-ltmethod-namegt-method-could-not-be-resolved-when-client-calls-method-on-server"></a>"Exceção: o método&gt; nome do método de &lt;não pôde ser resolvido" quando o cliente chama o método no servidor

Esse erro pode resultar do uso de tipos de dados que não podem ser descobertos em uma carga JSON, como matriz. A solução alternativa é usar um tipo de dados que seja detectável por JSON, como IList. Para obter mais informações, consulte o [cliente .net não pode chamar métodos de Hub com parâmetros de matriz](https://github.com/SignalR/SignalR/issues/2672).
...
Esse erro será visto se `MapSignalR` for chamado duas vezes pelo seu aplicativo. Alguns aplicativos de exemplo chamam `MapSignalR` diretamente na classe Startup; outros fazem a chamada em uma classe wrapper. Certifique-se de que seu aplicativo não faça ambos.

### <a name="websocket-is-not-used"></a>WebSocket não é usado

Se você verificou que o servidor e os clientes atendem aos requisitos do WebSocket (listado no documento [plataformas com suporte](../getting-started/supported-platforms.md) ), será necessário habilitar o WebSocket no servidor. As instruções para fazer isso podem ser encontradas [aqui](https://www.iis.net/learn/get-started/whats-new-in-iis-8/iis-80-websocket-protocol-support).

### <a name="connection-is-undefined"></a>$. a conexão está indefinida

Esse erro indica que os scripts em uma página não estão sendo carregados corretamente ou o proxy do Hub não está acessível ou está sendo acessado incorretamente. Verifique se as referências de script na sua página correspondem aos scripts carregados em seu projeto e se o/signalr/hubs pode ser acessado em um navegador quando o servidor estiver em execução.

### <a name="one-or-more-types-required-to-compile-a-dynamic-expression-cannot-be-found"></a>Um ou mais tipos necessários para compilar uma expressão dinâmica não podem ser encontrados

Esse erro indica que a biblioteca de `Microsoft.CSharp` está ausente. Adicione-o na guia **assemblies-&gt;Framework** .

### <a name="caller-state-cannot-be-accessed-from-clientscaller-in-visual-basic-or-in-a-strongly-typed-hub-conversion-from-type-taskof-object-to-type-string-is-not-valid-error"></a>O estado do chamador não pode ser acessado de clientes. chamador em Visual Basic ou em um hub fortemente tipado; "A conversão do tipo ' Task (Of Object) ' para o tipo ' String ' não é válida"

Para acessar o estado do chamador em Visual Basic ou em um hub fortemente tipado, use a propriedade `Clients.CallerState` (introduzida no Signalr 2,1) em vez de `Clients.Caller`.
...
Este comportamento ocorre por design. O signalr requer .NET Framework 4 ou posterior; Isso exige que os aplicativos Signalr sejam desenvolvidos no Visual Studio 2010 ou posterior. O componente de servidor do Signalr requer .NET Framework 4,5.
...
O signalr tem suporte no IIS 7,0 e 7,5, mas o suporte para URLs sem extensão deve ser adicionado. Para adicionar suporte a URLs sem extensão, consulte [https://support.microsoft.com/kb/980368](https://support.microsoft.com/kb/980368)

O signalr requer que o ASP.NET seja instalado no servidor (o ASP.NET não está instalado no IIS por padrão). Para instalar o ASP.NET, consulte [downloads do ASP.net](https://www.asp.net/downloads).
...
### <a name="fileloadexception-when-hosting-signalr-in-an-azure-worker-role"></a>FileLoadException ao hospedar o sinalizador em uma função de trabalho do Azure

O sinalização de hospedagem em uma função de trabalho do Azure pode resultar na exceção "não foi possível carregar o arquivo ou o assembly ' Microsoft. Owin, Version = 2.0.0.0". Esse é um problema conhecido com o NuGet; Os redirecionamentos de associação não são adicionados automaticamente aos projetos de função de trabalho do Azure. Para corrigir isso, você pode adicionar os redirecionamentos de associação manualmente. Adicione as seguintes linhas ao arquivo de `app.config` para seu projeto de função de trabalho.
...
Os tópicos usados pelo Azure backplane são mantidos internamente; Eles não se destinam a ser configuráveis pelo usuário.
