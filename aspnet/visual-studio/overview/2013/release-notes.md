...
Agora, o ASP.NET dá suporte à autenticação baseada em declarações, onde a identidade do usuário é representada como um conjunto de declarações de um emissor confiável. Os usuários podem ser autenticados usando um nome de usuário e a senha mantida em um banco de dados do aplicativo, ou usando provedores de identidade social (por exemplo: Microsoft contas, Facebook, Google, Twitter), ou usando as contas organizacionais por meio do Azure Active Directory ou serviços de Federação do Active Directory (ADFS).
...
2. **Request.Url ou RequestContext.Url é null durante uma solicitação em lote**
    ...
    Esse problema atualmente é rastreado aqui: [BatchRequestContext.Url é nulo para a solicitação de envio em lote](http://aspnetwebstack.codeplex.com/workitem/1301).
