...
- *key*: A chave do produto para a taxa.
...
No exemplo anterior, a ação está associada a uma única entidade: O cliente taxas de um único produto. Você também pode vincular uma ação para uma coleção de entidades. Basta fazer as seguintes alterações:
...
Por veja que isso é importante: Se uma ação for transitória, o servidor precisa dizer ao cliente quando a ação está disponível. Ele faz isso, incluindo um link para a ação na entidade. Aqui está um exemplo de uma entidade de filme:
