...
- `TodoController`: Manipula as solicitações HTTP para itens de tarefas pendentes
- `TodoListController`: Manipula as solicitações HTTP para listas de tarefas pendentes.
...
| DELETE /api/todo/*id* | `DeleteTodoList` | Exclui uma lista de tarefas Pendentes. |
...
- **visible**: Usado para alternar a visibilidade. Ocultar marcação quando uma coleção está vazia ou verifique a mensagem de erro visível.
