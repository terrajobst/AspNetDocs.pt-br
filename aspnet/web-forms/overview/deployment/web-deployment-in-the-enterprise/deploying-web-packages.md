...
Independentemente se você implanta o pacote da web executando o *. Deploy. cmd* do arquivo ou usando MSDeploy.exe diretamente, você precisa especificar um tipo de autenticação. A implantação da Web aceita dois valores possíveis: **NTLM** ou **básico**. Se você especificar a autenticação básica, você também precisará fornecer um nome de usuário e senha. Há vários fatores que você precisa estar ciente de quando você seleciona um tipo de autenticação:
