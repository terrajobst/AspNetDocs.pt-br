...
Para obter informações sobre como criar pacotes da web, consulte [compilação e empacotamento Web Application Projects](building-and-packaging-web-application-projects.md). Para obter orientação sobre como implantar, na verdade, um pacote da web, consulte [Implantando pacotes da Web](deploying-web-packages.md). Para obter uma explicação passo a passo sobre como criar uma *parameters.xml* arquivos, consulte [como: Use parâmetros para configurar as configurações quando um pacote de implantação está instalado](https://msdn.microsoft.com/library/ff398068.aspx).
