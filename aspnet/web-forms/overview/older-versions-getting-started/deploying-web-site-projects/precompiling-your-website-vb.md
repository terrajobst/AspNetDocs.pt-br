...
Comparar **Figura 3** à **Figura 5**. Embora as duas pastas podem parecer idênticas, observe que a pasta não atualizável de interface do usuário não tem a página mestra, `Site.master`. E enquanto **Figura 5** inclui várias páginas ASP.NET, se você exibir o conteúdo desses arquivos, você verá que eles já foram removidos da sua marcação declarativa e substituídos pelo texto do espaço reservado: "Isso é um arquivo de marcador gerado pela ferramenta de pré-compilação e não deve ser excluído!"
