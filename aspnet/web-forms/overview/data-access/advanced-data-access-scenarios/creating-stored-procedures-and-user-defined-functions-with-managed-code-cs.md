...
Para adicionar um novo procedimento armazenado ao projeto, clique com o botão direito do mouse no nome do projeto `ManagedDatabaseConstructs` e escolha Adicionar um novo procedimento armazenado. Nomeie o arquivo `GetProductsWithPriceLessThan.cs`. Como vimos na etapa 3, isso criará um novo C# arquivo de classe com um método chamado `GetProductsWithPriceLessThan` colocado dentro da classe `partial` `StoredProcedures`.
