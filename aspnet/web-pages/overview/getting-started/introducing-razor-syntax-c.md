...
>   
> 
> <span data-ttu-id="4b2b1-115">Este tutorial também funciona com ASP.NET Web Pages 2.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-115">This tutorial also works with ASP.NET Web Pages 2.</span></span>
...
<span data-ttu-id="4b2b1-162">Geralmente, você trabalhará com o `Request` do objeto, que oferece informações como os valores das caixas de texto (campos de formulário) na página, o tipo de navegador fez a solicitação, a URL da página, a identidade do usuário, etc. O exemplo a seguir mostra como acessar propriedades do `Request` objeto e como chamar o `MapPath` método o `Request` objeto, que fornece o caminho absoluto da página no servidor:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-162">You'll often work with the `Request` object, which gives you information like the values of text boxes (form fields) on the page, what type of browser made the request, the URL of the page, the user identity, etc. The following example shows how to access properties of the `Request` object and how to call the `MapPath` method of the `Request` object, which gives you the absolute path of the page on the server:</span></span>
...
<span data-ttu-id="4b2b1-283">Para marcação, como você provavelmente sabe, você pode criar um comentário HTML:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-283">For markup, as you probably know, you can create an HTML comment:</span></span>
...
<span data-ttu-id="4b2b1-367">Aqui está um exemplo para ajudá-lo a entender as diferenças:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-367">Here's an example to help you understand the differences:</span></span>
...
<span data-ttu-id="4b2b1-399">Código de servidor do ASP.NET permite que você execute tarefas com base nas condições e escrever código que repete instruções um número específico de vezes (ou seja, código que executa um loop).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-399">ASP.NET server code lets you perform tasks based on conditions and write code that repeats statements a specific number of times (that is, code that runs a loop).</span></span>
...
<span data-ttu-id="4b2b1-411">Para testar um grande número de condições, use um `switch` bloco:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-411">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="4b2b1-412">O valor a ser testado está entre parênteses (no exemplo, o `weekday` variável).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-412">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="4b2b1-413">Cada teste individual usa um `case` instrução que termina com dois-pontos (:).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-413">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="4b2b1-414">Se o valor de um `case` instrução corresponde ao valor de teste, o código daquele bloco case é executado.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-414">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="4b2b1-415">Fechar cada instrução case com uma `break` instrução.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-415">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="4b2b1-416">(Se você se esquecer de incluir quebra em cada `case` bloquear, o código do próximo `case` instrução serão executados também.) Um `switch` bloco geralmente tem uma `default` instrução como o último caso de uma &quot;todo o resto&quot; opção que é executado se nenhum dos outros casos forem verdadeiras.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-416">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="4b2b1-417">O resultado dos dois últimos blocos condicionais exibido em um navegador:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-417">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="4b2b1-419">Código de loop</span><span class="sxs-lookup"><span data-stu-id="4b2b1-419">Looping Code</span></span>

<span data-ttu-id="4b2b1-420">Geralmente, você precisará executar as mesmas instruções repetidas vezes.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-420">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="4b2b1-421">Você pode fazer isso por meio de loops.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-421">You do this by looping.</span></span> <span data-ttu-id="4b2b1-422">Por exemplo, você sempre execute as mesmas instruções para cada item em uma coleção de dados.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-422">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="4b2b1-423">Se você souber exatamente quantas vezes você deseja executar um loop, você pode usar um `for` loop.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-423">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="4b2b1-424">Esse tipo de loop é especialmente útil para de contagem ou contagem regressiva:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-424">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="4b2b1-425">O loop começa com o `for` palavra-chave, seguido de três instruções entre parênteses, cada encerrada com ponto e vírgula.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-425">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="4b2b1-426">Dentro dos parênteses, a primeira instrução (`var i=10;`) cria um contador e o inicializa para 10.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-426">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="4b2b1-427">Não é necessário nomear o contador `i` &#8212; você pode usar qualquer variável.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-427">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="4b2b1-428">Quando o `for` loop é executado, o contador é incrementado automaticamente.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-428">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="4b2b1-429">A segunda instrução (`i < 21;`) define a condição para até onde você deseja contar.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-429">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="4b2b1-430">Nesse caso, você deseja que ele vá até um máximo de 20 (ou seja, continue enquanto o contador for menor que 21).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-430">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="4b2b1-431">A terceira instrução (`i++` ) usa um operador de incremento, que só especifica que o contador deve ter 1 adicionado a cada vez que o loop é executado.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-431">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="4b2b1-432">Dentro das chaves é o código que será executado em cada iteração do loop.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-432">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="4b2b1-433">A marcação cria um novo parágrafo (`<p>` elemento) cada vez e adiciona uma linha na saída, exibindo o valor de `i` (o contador).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-433">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="4b2b1-434">Quando você executa essa página, o exemplo cria 11 linhas exibindo a saída, com o texto em cada linha que indica o número de item.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-434">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="4b2b1-436">Se você estiver trabalhando com uma coleção ou matriz, você geralmente usa um `foreach` loop.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-436">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="4b2b1-437">Uma coleção é um grupo de objetos semelhantes e o `foreach` loop permite que você executar uma tarefa em cada item na coleção.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-437">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="4b2b1-438">Esse tipo de loop é conveniente para coleções, pois ao contrário de um `for` loop, você não precisa incrementar o contador ou definir um limite.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-438">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="4b2b1-439">Em vez disso, o `foreach` loop código simplesmente ocorrerá por meio da coleção, até que ela seja concluída.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-439">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="4b2b1-440">Por exemplo, o código a seguir retorna os itens a `Request.ServerVariables` coleção, que é um objeto que contém informações sobre seu servidor web.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-440">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="4b2b1-441">Ele usa um `foreac` loop h para exibir o nome de cada item, criando um novo `<li>` elemento em uma lista com marcadores de HTML.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-441">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="4b2b1-442">O `foreach` palavra-chave é seguido por parênteses, em que você declare uma variável que representa um único item na coleção (no exemplo, `var item`), seguido pelo `in` palavra-chave, seguido pela coleção que você deseja fazer o loop.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-442">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="4b2b1-443">No corpo do `foreach` loop, você pode acessar o item atual usando a variável declarada anteriormente por você.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-443">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="4b2b1-445">Para criar um loop de propósito mais geral, use o `while` instrução:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-445">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="4b2b1-446">Um `while` loop começa com o `while` palavra-chave, seguido por parênteses, onde você especifica por quanto tempo o loop continua (aqui, para desde que `countNum` é menor que 50), em seguida, o bloco de repetir.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-446">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="4b2b1-447">Loops normalmente incrementam (Adicionar ao) ou decrementar (subtrair de) uma variável ou um objeto usado para contagem.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-447">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="4b2b1-448">No exemplo, o `+=` operador adiciona 1 ao `countNum` cada vez que o loop é executado.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-448">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="4b2b1-449">(Para diminuir a uma variável em um loop de contagem regressiva, você usaria o operador de decremento `-=`).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-449">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="4b2b1-450">Objetos e coleções</span><span class="sxs-lookup"><span data-stu-id="4b2b1-450">Objects and Collections</span></span>

<span data-ttu-id="4b2b1-451">Quase tudo em um site ASP.NET é um objeto, incluindo a própria página da web.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-451">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="4b2b1-452">Esta seção discute alguns objetos importantes que você trabalhará com frequência em seu código.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-452">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="4b2b1-453">Objetos de página</span><span class="sxs-lookup"><span data-stu-id="4b2b1-453">Page Objects</span></span>

<span data-ttu-id="4b2b1-454">O objeto mais básico no ASP.NET é a página.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-454">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="4b2b1-455">Você pode acessar as propriedades do objeto page diretamente sem qualquer objeto qualificado.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-455">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="4b2b1-456">O código a seguir obtém o caminho do arquivo da página, usando o `Request` objeto da página:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-456">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="4b2b1-457">Para torná-lo claro que você está fazendo referência a propriedades e métodos no objeto da página atual, você pode usar a palavra-chave `this` para representar o objeto de página em seu código.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-457">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="4b2b1-458">Aqui está o exemplo de código anterior, com `this` adicionado para representar a página:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-458">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="4b2b1-459">Você pode usar propriedades do `Page` objeto do qual obter muitas informações, tais como:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-459">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- `Request`<span data-ttu-id="4b2b1-460">.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-460">.</span></span> <span data-ttu-id="4b2b1-461">Como você já viu, esta é uma coleção de informações sobre a solicitação atual, incluindo o tipo de navegador fez a solicitação, a URL da página, a identidade do usuário, etc.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-461">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- `Response`<span data-ttu-id="4b2b1-462">.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-462">.</span></span> <span data-ttu-id="4b2b1-463">Esta é uma coleção de informações sobre a resposta (página) que será enviada ao navegador quando o código de servidor concluiu a execução.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-463">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="4b2b1-464">Por exemplo, você pode usar essa propriedade para gravar informações na resposta.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-464">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="4b2b1-465">Objetos de coleção (matrizes e dicionários)</span><span class="sxs-lookup"><span data-stu-id="4b2b1-465">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="4b2b1-466">Um *coleta* é um grupo de objetos do mesmo tipo, como uma coleção de `Customer` objetos de banco de dados.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-466">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="4b2b1-467">ASP.NET contém várias coleções internas, como o `Request.Files` coleção.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-467">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="4b2b1-468">Geralmente, você trabalhará com dados em coleções.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-468">You'll often work with data in collections.</span></span> <span data-ttu-id="4b2b1-469">Dois tipos de coleção comuns são as *array* e o *dicionário*.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-469">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="4b2b1-470">Uma matriz é útil quando você deseja armazenar uma coleção de itens semelhantes, mas não quiser criar uma variável separada para manter cada item:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-470">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="4b2b1-471">Com matrizes, você declara um tipo de dados específico, como `string`, `int`, ou `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-471">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="4b2b1-472">Para indicar que a variável pode conter uma matriz, adicione parênteses para a declaração (como `string[]` ou `int[]`).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-472">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="4b2b1-473">Você pode acessar itens em uma matriz usando sua posição (índice) ou usando o `foreach` instrução.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-473">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="4b2b1-474">Índices de matriz são baseados em zero &#8212; ou seja, o primeiro item está na posição 0, o segundo item estiver na posição 1 e assim por diante.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-474">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="4b2b1-475">Você pode determinar o número de itens em uma matriz obtendo sua `Length` propriedade.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-475">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="4b2b1-476">Para obter a posição de um item específico na matriz (para pesquisar a matriz), use o `Array.IndexOf` método.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-476">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="4b2b1-477">Você também pode fazer coisas como reverter o conteúdo de uma matriz (o `Array.Reverse` método) ou classificar o conteúdo (o `Array.Sort` método).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-477">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="4b2b1-478">A saída do código de matriz de cadeia de caracteres exibido em um navegador:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-478">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="4b2b1-480">Um dicionário é uma coleção de pares chave/valor, em que você fornece a chave (ou nome) para definir ou recuperar o valor correspondente:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-480">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="4b2b1-481">Para criar um dicionário, você deve usar o `new` palavra-chave para indicar que você está criando um novo objeto de dicionário.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-481">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="4b2b1-482">Você pode atribuir um dicionário para uma variável usando o `var` palavra-chave.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-482">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="4b2b1-483">Você indica os tipos de dados dos itens no dicionário usando colchetes angulares ( `< >` ).</span><span class="sxs-lookup"><span data-stu-id="4b2b1-483">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="4b2b1-484">No final da declaração, você deve adicionar um par de parênteses, porque isso é, na verdade, um método que cria um novo dicionário.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-484">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="4b2b1-485">Para adicionar itens ao dicionário, você pode chamar o `Add` método da variável de dicionário (`myScores` nesse caso) e, em seguida, especifique uma chave e um valor.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-485">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="4b2b1-486">Como alternativa, você pode usar colchetes para indicar a chave e fazer uma simples atribuição, como no exemplo a seguir:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-486">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="4b2b1-487">Para obter um valor do dicionário, você pode especificar a chave entre colchetes:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-487">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="4b2b1-488">Chamar métodos com parâmetros</span><span class="sxs-lookup"><span data-stu-id="4b2b1-488">Calling Methods with Parameters</span></span>

<span data-ttu-id="4b2b1-489">Ao ler no início deste artigo, os objetos que você programa com podem ter métodos.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-489">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="4b2b1-490">Por exemplo, uma `Database` objeto pode ter um `Database.Connect` método.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-490">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="4b2b1-491">Muitos métodos também tem um ou mais parâmetros.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-491">Many methods also have one or more parameters.</span></span> <span data-ttu-id="4b2b1-492">Um *parâmetro* é um valor que você passa para um método para habilitar o método concluir a tarefa.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-492">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="4b2b1-493">Por exemplo, examinar uma declaração para o `Request.MapPath` método, que usa três parâmetros:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-493">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="4b2b1-494">(A linha foi encapsulada para torná-lo mais legível.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-494">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="4b2b1-495">Lembre-se de que você pode colocar quebras de linha quase qualquer lugar exceto inside cadeias de caracteres que são colocados entre aspas.)</span><span class="sxs-lookup"><span data-stu-id="4b2b1-495">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="4b2b1-496">Esse método retorna o caminho físico no servidor que corresponde a um caminho virtual especificado.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-496">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="4b2b1-497">Os três parâmetros para o método são `virtualPath`, `baseVirtualDir`, e `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-497">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="4b2b1-498">(Observe que na declaração, os parâmetros são listados com os tipos de dados dos dados que ele aceitará.) Quando você chama esse método, você deve fornecer valores para todos os três parâmetros.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-498">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="4b2b1-499">A sintaxe do Razor oferece duas opções para passar parâmetros para um método: *parâmetros posicionais* e *parâmetros nomeados*.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-499">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="4b2b1-500">Para chamar um método usando parâmetros posicionais, você pode passar os parâmetros em uma ordem estrita que é especificada na declaração de método.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-500">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="4b2b1-501">(Você normalmente saberia nesta ordem, lendo a documentação do método.) Você deve seguir a ordem, e você não pode ignorar qualquer um dos parâmetros &#8212; se necessário, você passa uma cadeia de caracteres vazia (`""`) ou `null` para um parâmetro posicional que você não tiver um valor para.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-501">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="4b2b1-502">O exemplo a seguir pressupõe que você tem uma pasta chamada *scripts* em seu site.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-502">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="4b2b1-503">O código chama o `Request.MapPath` e passa valores para os três parâmetros na ordem correta.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-503">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="4b2b1-504">Ele, em seguida, exibe o caminho de mapeada resultante.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-504">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="4b2b1-505">Quando um método tem muitos parâmetros, você pode manter seu código mais legível usando parâmetros nomeados.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-505">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="4b2b1-506">Para chamar um método usando parâmetros nomeados, você especifica o nome do parâmetro seguido por dois-pontos (:) e, em seguida, o valor.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-506">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="4b2b1-507">A vantagem dos parâmetros nomeados é que você pode passá-los em qualquer ordem desejada.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-507">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="4b2b1-508">(Uma desvantagem é que a chamada de método não é mais compacta.)</span><span class="sxs-lookup"><span data-stu-id="4b2b1-508">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="4b2b1-509">O exemplo a seguir chama o método acima, mas usa parâmetros para fornecer os valores nomeados:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-509">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="4b2b1-510">Como você pode ver, os parâmetros são passados em uma ordem diferente.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-510">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="4b2b1-511">No entanto, se você executar o exemplo anterior e este exemplo, eles retornar o mesmo valor.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-511">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="4b2b1-512">Manipulando erros</span><span class="sxs-lookup"><span data-stu-id="4b2b1-512">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="4b2b1-513">Instruções Try-Catch</span><span class="sxs-lookup"><span data-stu-id="4b2b1-513">Try-Catch Statements</span></span>

<span data-ttu-id="4b2b1-514">Geralmente, você terá as instruções em seu código que pode falhar por razões de fora do seu controle.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-514">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="4b2b1-515">Por exemplo:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-515">For example:</span></span>

- <span data-ttu-id="4b2b1-516">Se seu código tentar criar ou acessar um arquivo, todos os tipos de erros podem ocorrer.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-516">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="4b2b1-517">O arquivo que você deseja que pode não existir, ele pode ter sido bloqueado, o código pode não ter permissões e assim por diante.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-517">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="4b2b1-518">Da mesma forma, se seu código tenta atualizar registros em um banco de dados, pode haver problemas de permissões, a conexão ao banco de dados pode ser descartado, os dados para salvar podem estar inválido e assim por diante.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-518">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="4b2b1-519">Em termos de programação, essas situações são chamadas *exceções*.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-519">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="4b2b1-520">Se seu código encontrar uma exceção, ele gera (gera) uma mensagem de erro que está, na melhor das hipóteses, irritantes aos usuários:</span><span class="sxs-lookup"><span data-stu-id="4b2b1-520">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="4b2b1-522">Em situações em que seu código poderá encontrar exceções e para evitar mensagens de erro desse tipo, você pode usar `try/catch` instruções.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-522">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="4b2b1-523">No `try` instrução, que você executa o código que você está verificando.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-523">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="4b2b1-524">Em uma ou mais `catch` instruções, você pode procurar específicos de erros (tipos específicos de exceções) que possam ter ocorrido.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-524">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="4b2b1-525">Você pode incluir tantos `catch` instruções que você precisam procurar por erros que você está prevendo.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-525">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="4b2b1-526">É recomendável que você evite usar o `Response.Redirect` método no `try/catch` instruções, porque isso pode causar uma exceção em sua página.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-526">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>


<span data-ttu-id="4b2b1-527">O exemplo a seguir mostra uma página que cria um arquivo de texto na primeira solicitação e, em seguida, exibe um botão que permite que o usuário abrir o arquivo.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-527">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="4b2b1-528">O exemplo deliberadamente usa um nome de arquivo incorreto para que ela fará com que uma exceção.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-528">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="4b2b1-529">O código inclui `catch` instruções para duas exceções possíveis: `FileNotFoundException`, que ocorre se o nome do arquivo é ruim, e `DirectoryNotFoundException`, que ocorre se o ASP.NET ainda não é possível localizar a pasta.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-529">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="4b2b1-530">(Você pode remover o comentário uma instrução no exemplo a fim de ver como ele é executado quando tudo está funcionando corretamente.)</span><span class="sxs-lookup"><span data-stu-id="4b2b1-530">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="4b2b1-531">Se seu código não trata a exceção, você verá uma página de erro, como a captura de tela anterior.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-531">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="4b2b1-532">No entanto, o `try/catch` seção ajuda a impedir que o usuário ver esses tipos de erros.</span><span class="sxs-lookup"><span data-stu-id="4b2b1-532">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="4b2b1-533">Recursos adicionais</span><span class="sxs-lookup"><span data-stu-id="4b2b1-533">Additional Resources</span></span>

**<span data-ttu-id="4b2b1-534">Programação com o Visual Basic</span><span class="sxs-lookup"><span data-stu-id="4b2b1-534">Programming with Visual Basic</span></span>**


[<span data-ttu-id="4b2b1-535">Apêndice: Sintaxe e a linguagem Visual Basic</span><span class="sxs-lookup"><span data-stu-id="4b2b1-535">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)


**<span data-ttu-id="4b2b1-536">Documentação de referência</span><span class="sxs-lookup"><span data-stu-id="4b2b1-536">Reference Documentation</span></span>**


[<span data-ttu-id="4b2b1-537">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="4b2b1-537">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="4b2b1-538">Linguagem C#</span><span class="sxs-lookup"><span data-stu-id="4b2b1-538">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
